...
"""
    ...
    def start_game(self):
        ...
        self.player_img = PhotoImage(file=os.path.join(IMG_DIR, "Girl.png")).zoom(3).subsample(2)
        self.bullet_img = PhotoImage(file=os.path.join(IMG_DIR, "heart.png"))

        self.enemy_imgs = {
            1: PhotoImage(file=os.path.join(IMG_DIR, "Hat_man1.png")).subsample(2),
            5: PhotoImage(file=os.path.join(IMG_DIR, "5s.png")).zoom(3).subsample(2),
        ...
        try:
            ...
            self.shoot_sfx = pygame.mixer.Sound(os.path.join(SOUND_DIR, "hit.mp3")) 
            self.hit_sfx   = pygame.mixer.Sound(os.path.join(SOUND_DIR, "hit.mp3"))
        except:
            self.shoot_sfx = None
            self.hit_sfx = None
        ...
        self.enemy_lines = {
            1:["이 편지 내 거야!","넌 평생 고백 못해!","너는 그 애와 어울리지 않아!","넌 바보군 크흑","고백은 포기하도록!","히히! 마음 먼저 가져간다~","사랑은 타이밍이라던데? 넌 늦었어!","편지 없어져도 울진 않겠지?","몰래 가져가볼까~?"],
            5:["고백…실패…","남자는 좋아하는 여자 앞에성 그냥이라고는 없어. 언제나 반드시 이유가 있지.","떨려서 말 못 했지? 괜찮아~ 난 너 같은 애 전용 요정이거든!","네 용기는 0, 실패 확률은 100%! 완벽하다!"],
            10:["잊은 줄 알았어…","썸…이었나?","하얀 천이랑 바람만 있으면 어디든 갈 수 있어","나도 남자야! 너란 여자를 죽도록 안고싶어하는 남자 맞다구!!"],
            50:["더 이상 피하지 않으려고, 한 번 포기하면 얼마나 후회막급인지 누구덕분에 알게됐거든","네 마음… 따뜻하구나","넌 가능성이 있다"]
        }
    ...
    def show_stage_text(self):
        self.canvas.delete("all")
        bg = self.bg1 if self.stage == 1 else self.bg2
        self.canvas.create_image(0,0,anchor=NW,image=bg)
        ...
        for e in self.enemies[:]:
            ...
            for b in self.bullets[:]:
                if self.collide(img,b):
                    # 적의 현재 좌표를 가져옵니다.
                    enemy_coords = self.canvas.coords(img)
                    ...
                    # 랜덤 대사를 선택하고 화면에 표시합니다.
                    line_text = random.choice(self.enemy_lines[s])
                    # 텍스트에 흰색 테두리를 추가하여 가독성을 높입니다.
                    outline_id = self.canvas.create_text(enemy_coords[0]+1, enemy_coords[1]+1, text=line_text, font=("Arial", 14, "bold"), fill="white")
                    text_id = self.canvas.create_text(enemy_coords[0], enemy_coords[1], text=line_text, font=("Arial", 14, "bold"), fill="black")
                    
                    # 1초 후에 대사와 테두리를 삭제합니다.
                    self.window.after(1000, lambda: self.canvas.delete(text_id))
                    self.window.after(1000, lambda: self.canvas.delete(outline_id))
